...
Use of this Software is limited and subject to the ANT SoftDevice License Agreement.
The Agreement accompanies the Software in the root directory of the .zip file containing this Software.
**/
...
// ////////////////////////////////////////////
#define ANT_VERSION_STRING "BGI7.00.01"  ///< ANT version number
 ...
 *  @{ */
// ////////////////////////////////////////////
#define MAX_ANT_CHANNELS                           ((uint8_t)15) ///< The maximum number of ANT channels
#define SIZE_OF_NONENCRYPTED_ANT_CHANNEL           ((uint8_t)120)
#define SIZE_OF_ENCRYPTED_KEY                      ((uint8_t)16)
#define SIZE_OF_ENCRYPTED_ANT_CHANNEL              ((uint8_t)37)
#define SIZE_OF_ANT_EVENT                          ((uint8_t)2)
...
#define GET_ANT_ENCRYPTED_CHANNEL_SIZE(ucNumberOfEncryptedChannels) \
   ((ucNumberOfEncryptedChannels > 0) ? ((ucNumberOfEncryptedChannels * SIZE_OF_ENCRYPTED_ANT_CHANNEL) - SIZE_OF_ENCRYPTED_KEY) : 0)

#define GET_ANT_EVENT_QUEUE_SIZE(usNumberOfEvents) \
   ((usNumberOfEvents > MIN_ANT_EVENT_QUEUE_SIZE) ? (usNumberOfEvents * SIZE_OF_ANT_EVENT) : 0)

 /*!
  * \def ANT_ENABLE_GET_REQUIRED_SPACE(ucTotalNumberOfChannels, ucNumberOfEncryptedChannels, usTxQueueByteSize, usNumberOfEvents)
  * This macro will generate the required amount of memory to be allocated for the ANT stack based on the parameters entered.
  *
  * ucTotalNumberOfChannels is an unsigned char (1 octet) denoting the total number of ANT channels desired (1 to @ref MAX_ANT_CHANNELS, defined in ant_parameters.h)
  * ucTotalNumberOfEncryptedChannels is an unsigned char (1 octet) denoting the total number of ANT channels (0 to ucTotalNumberOfChannels) that support encryption
  * usTxQueueByteSize is an unsigned short (2 octets) denoting the size of the transmit burst queue. Must be greater than or equal to @ref MIN_ANT_TX_BURST_QUEUE_SIZE and a power of 2 (ie 64, 128, 256, etc).
  * usEvtQueueSize is an unsigned short (2 octets) denoting the number of events in the event queue. Anything less than @ref MIN_ANT_EVENT_QUEUE_SIZE will use the internal event queue memory.
  *
  * Returns the memory required for the features requested. Used with sd_ant_enable().
  */
#define ANT_ENABLE_GET_REQUIRED_SPACE(ucTotalNumberOfChannels, ucNumberOfEncryptedChannels, usTxQueueByteSize, usNumberOfEvents) \
   GET_ANT_CHANNEL_SIZE(ucTotalNumberOfChannels) +                                                                               \
   GET_ANT_ENCRYPTED_CHANNEL_SIZE(ucNumberOfEncryptedChannels) +                                                                 \
   GET_ANT_TX_BURST_QUEUE_SIZE(usTxQueueByteSize) +                                                                              \
   GET_ANT_EVENT_QUEUE_SIZE(usNumberOfEvents)
/** @} */
...
#define CAPABILITIES_DATA_FILTERING_ENABLED                 ((uint8_t)0x02) ///< Bitfield in CAPABILITIES_ADVANCED_4 byte indicating data filtering support.
#define CAPABILITIES_SEARCH_UPLINK_ENABLED                  ((uint8_t)0x04) ///< Bitfield in CAPABILITIES_ADVANCED_4 byte indicating search uplink support.
#define CAPABILITIES_GROUP_TRANSMITTER_INITIATION_ENABLED   ((uint8_t)0x08) ///< Bitfield in CAPABILITIES_ADVANCED_4 byte indicating group transmitter initiation support.
#define CAPABILITIES_TIME_BASE_ENABLED                      ((uint8_t)0x10) ///< Bitfield in CAPABILITIES_ADVANCED_4 byte indicating extended time base support.
#define CAPABILITIES_TIME_SYNC_ENABLED                      ((uint8_t)0x20) ///< Bitfield in CAPABILITIES_ADVANCED_4 byte indicating time sync support.
#define CAPABILITIES_PA_LNA_SUPPORT_ENABLED                 ((uint8_t)0x40) ///< Bitfield in CAPABILITIES_ADVANCED_4 byte indicating PA/LNA support.
/** @} */

// ////////////////////////////////////////////
/** @name Advanced capabilities 5 defines
 *  @{ */
// ////////////////////////////////////////////
#define CAPABILITIES_CHANNEL_START_OFFSET_ENABLED           ((uint8_t)0x01) ///< Bitfield in CAPABILITIES_ADVANCED_5 byte indicating channel start offset support.
#define CAPABILITIES_SEARCHING_CHANNEL_UPLINK_ENABLED       ((uint8_t)0x02) ///< Bitfield in CAPABILITIES_ADVANCED_5 byte indicating background searching channel uplink support.
#define CAPABILITIES_ID_MATCH_FIX_ON_SHARED_CHANNEL_RXBURST ((uint8_t)0x04) ///< Bitfield in CAPABILITIES_ADVANCED_5 byte indicating id match fix for shared channel rx burst transfer.
/** @} */
...
// ////////////////////////////////////////////
/** @name Channel Allocated Spacing Defines
 *  @{ */
// ////////////////////////////////////////////
#define ALLOCATED_SPACING_TYPE_1                   ((uint16_t)164) ///< Spacing allocation for uni-directional broadcast message spacing, ~5.0ms
#define ALLOCATED_SPACING_TYPE_2                   ((uint16_t)263) ///< Spacing allocation for bi-directional broadcast message spacing, ~8.0ms
#define ALLOCATED_SPACING_TYPE_3                   ((uint16_t)279) ///< Spacing allocation for uni-directional acknowledged message spacing, ~8.5ms
#define ALLOCATED_SPACING_TYPE_4                   ((uint16_t)378) ///< Spacing allocation for bi-directional acknowledged message spacing, ~11.5ms

#define ALLOCATED_SPACING_MIN                      ALLOCATED_SPACING_TYPE_1
/** @} */

// ////////////////////////////////////////////
/** @name set DC to DC Defines
 *  @{ */
// ////////////////////////////////////////////
#define DC_TO_DC_OFF                               ((uint8_t)0x00) ///< Turn DC to DC off
#define DC_TO_DC_ON                                ((uint8_t)0x01) ///< Turn DC to DC on
/** @} */

// ////////////////////////////////////////////
/** @name High Duty Search Defines
 *  @{ */
// ////////////////////////////////////////////
#define HIGH_DUTY_SEARCH_DISABLE                   ((uint8_t)0x00) ///< Disable high duty search
#define HIGH_DUTY_SEARCH_ENABLE                    ((uint8_t)0x01) ///< Enable high duty search
#define HIGH_DUTY_SEARCH_SUPPRESSION_NONE          ((uint8_t)0x00) ///< No high priority search supression cycles.
#define HIGH_DUTY_SEARCH_SUPPRESSION_DEFAULT       ((uint8_t)0x03) ///< Default setting for search suppression (750ms everty 1.25s)
#define HIGH_DUTY_SEARCH_SUPPRESSION_FULL          ((uint8_t)0x05) ///< Full suppression of high priority search.

#define HIGH_DUTY_SEARCH_RESTART_INTERVAL_DEFAULT  ((uint16_t)56)  ///< Default restart interval (~1.7ms) used by high duty search
/** @} */

// ////////////////////////////////////////////
/** @name CRC Mode Defines
 *  @{ */
// ////////////////////////////////////////////
#define CRC_MODE_STANDARD_ANT                      ((uint8_t)0x00) ///< Standard CRC Mode used for ANT.
#define CRC_MODE_3BYTE                             ((uint8_t)0x01) ///< Extended 3-byte mode.

#define CRC_MODE_DEFAULT                           CRC_MODE_STANDARD_ANT
/** @} */

// ////////////////////////////////////////////
/** @name Channel Start Offset Defines
 *  @{ */
// ////////////////////////////////////////////
#define CHANNEL_START_OFFSET_NONE                  ((uint16_t)0x0000) ///< No offset specified
#define CHANNEL_START_OFFSET_MIN                   ((uint16_t)0x010A) ///< Minimum channel start offset configuration (~8.12ms)
#define CHANNEL_START_OFFSET_MAX                   ((uint16_t)0x7FFF) ///< Maximum channel start offset configuration (~1s)

#define CHANNEL_START_OFFSET_DEFAULT               CHANNEL_START_OFFSET_NONE
/** @} */
...
#define EVENT_RX_DATA_OVERFLOW                     ((uint8_t)0x0B) ///< ANT stack generated event when data has been blocked due to latency in application event servicing
//...
 ...
 *
 * BAUDRATE_TYPE used as index in asBaudLookup[] array
 * BAUDRATE_TYPE used as left bitshift into BAUD_SUPPORTED_BITFIELD bitfield
 * @{ */
// ////////////////////////////////////////////
typedef enum {
    BAUDRATE_TYPE_Baud1200,   // 1200 baud.
    BAUDRATE_TYPE_Baud2400,   // 2400 baud.
    BAUDRATE_TYPE_Baud4800,   // 4800 baud.
    BAUDRATE_TYPE_Baud9600,   // 9600 baud.
    BAUDRATE_TYPE_Baud19200,  // 19200 baud.
    BAUDRATE_TYPE_Baud38400,  // 38400 baud.
    BAUDRATE_TYPE_Baud50000,  // 50000 baud.
    BAUDRATE_TYPE_Baud57600,  // 57600 baud.
    BAUDRATE_TYPE_Baud115200, // 115200 baud.
    BAUDRATE_TYPE_Baud230400, // 230400 baud.
    BAUDRATE_TYPE_Baud460800, // 460800 baud.
    BAUDRATE_TYPE_Baud921600, // 921600 baud.
} BAUDRATE_TYPE;
/** @} */


/* Start of ANT Internal
*/
...
/** @defgroup ANT_TIME_BASE Time Base Defines
 *  @{ */
...
#define ANT_TIME_BASE_ALT2     ((uint8_t)0x02) ///< RTC2 (4 byte time stamp), not available on nRF52810
/** @} */


/** @brief Time stamp configuration structure */
typedef struct
{
   uint8_t ucTimeBase;         ///< Time base @ref ANT_TIME_BASE
   bool bTimeStampEnabled;   ///< False: Time stamp NOT used, True: Time stamp is used
...
{
   uint8_t ucTimeBase;             ///< Time base @ref ANT_TIME_BASE
   bool bInvalidationEnabled;      ///< False: Invalidation is NOT used, True: Invalidation is used
   uint8_t ucInvalidationByte;     ///< The byte used to indicate an invalid time sync packet
...
/**
 * @brief Used with the get/set high duty search config commands.
 */
typedef struct
{
   uint8_t bEnable;                    ///< Enable or disable high duty search. See HIGH_DUTY_SEARCH defines in ant_parameters.h
   uint8_t ucSearchSuppressionWindows; ///< Number of windows to apply suppression for. See HIGH_DUTY_SEARCH defines in ant_parameters.h

   uint16_t usRestartInterval;         ///< Restart interval. Use default value of HIGH_DUTY_SEARCH_RESTART_INTERVAL_DEFAULT unless otherwise specified. See HIGH_DUTY_SEARCH defines in ant_parameters.h
                                     ///< Restart interval is reset back to default when high duty search is disabled
} ANT_HIGH_DUTY_SEARCH_CONFIG;
...
/**
 * @brief The structure that holds configuration for PA/LNA GPIO outputs during radio events
 */
typedef struct {
   struct
   {
      bool bEnabled;          ///< Indicates that PA pin toggling is enabled
      uint8_t ucGPIO;         ///< Selects the GPIO used to control the PA
      bool bActiveState;      ///< Active state. 1 - Active high, 0 - Active low
   } PA_CONFIG;
   struct
   {
      bool bEnabled;          ///< Indicates that LNA pin toggling is enabled
      uint8_t ucGPIO;         ///< Selects the GPIO used to control the LNA
      bool bActiveState;      ///< Active state. 1 - Active high, 0 - Active low
   } LNA_CONFIG;
   uint8_t ucGPIOTECh;        ///< GPIOTE channel used to control PA and LNA pins
   uint8_t ucPPIChEnable;     ///< PPI channel used to control the PA/LNA signal before EVENTS_READY. Must be different from ucPPIChDisable
   uint8_t ucPPIChDisable;    ///< PPI channel used to control the PA/LNA signal during EVENTS_DISABLED. Must be different from ucPPIChEnable
} ANT_PA_LNA_CONFIG;
...
#define ANT_EXT_MESG_RSSI_FIELD_SIZE         ((uint8_t)3)
#define ANT_EXT_MESG_TIME_STAMP_FIELD_SIZE   ((uint8_t)2)
#define ANT_EXT_MESG_ALT_TIME_STAMP_FIELD_SIZE ((uint8_t)4)
#define ANT_EXT_STRING_SIZE                  ((uint8_t)17) // additional buffer to accommdate for 24 byte advance burst mode & encrypted usr data
...
#define MESG_MAX_EXT_DATA_SIZE               (ANT_EXT_MESG_DEVICE_ID_FIELD_SIZE + ANT_EXT_MESG_RSSI_FIELD_SIZE + ANT_EXT_MESG_ALT_TIME_STAMP_FIELD_SIZE + ANT_EXT_STRING_SIZE) // ANT device ID (4 bytes) + ANT RSSI (3 bytes) + ANT timestamp (4 bytes) + ANT Ext String Size
...
#define MESG_BUFFER_SIZE                     (MESG_SIZE_SIZE + MESG_ID_SIZE + MESG_CHANNEL_NUM_SIZE + MESG_MAX_DATA_SIZE + MESG_CHECKSUM_SIZE) ///< Message buffer size
...
#define MESG_CHANNEL_CRC_MODE_ID             ((uint8_t)0x58) ///< ANT stack - channel CRC mode message ID
...
#define MESG_HIGH_DUTY_SEARCH_MODE_ID        ((uint8_t)0x77) ///< ANT stack - high duty search config message ID
...
#define MESG_SET_SEARCH_CH_PRIORITY_SIZE              ((uint8_t)2)
#define MESG_HIGH_DUTY_SEARCH_MODE_EN_SIZE            ((uint8_t)2)
...
/**
 * @brief The structure that holds ANT messages
 */
typedef union ANT_MESSAGE
{
